...
## The DB relational schema

```{r}
library(knitr)
include_graphics("chinook.png")
```
...
ggplot(usa_genres_most_sold, aes(fct_reorder(genre, total), total)) +
  geom_col(aes(fill=total), show.legend = FALSE) + coord_flip() + labs(title="Top genres in USA", x="Total tracks sold", y="Genre") +
  geom_text (aes(label=sprintf("%1.1f%%",round(perc_sold,4)*100)), size=3.5, nudge_y=25) + scale_fill_viridis_c()
...
# employee that you find are relevant to the analysis. Create a plot of the
# results of your query. Write a short statement describing your results, and
...
```{r}
ggplot(ss_perf, aes(name, total_sold)) + geom_col(fill="steelblue") + labs(title = "Total tracks sold by employee", x="", y="Total")
```
...
```{r}
# For each country:
# use country value from the customers table
# Total number of customers
# Total value of sales
# Average value of sales per customer
# Average order value
# Where a country has only one customer, collect them into an "Other" group. The
# results should be sorted by the total sales from highest to lowest, with the
...
```{r}
ggplot(country_sales) + geom_col(mapping = aes(reorder(country,tot_sold), tot_sold, fill = tot_sold), show.legend = FALSE) + scale_y_continuous(labels = label_dollar()) + theme(axis.text.x = element_text(angle = 90)) + scale_fill_viridis_c() + labs(title="Total sales per country", subtitle = "Number of customers per country", y="", x ="") + geom_text(aes(x =reorder(country,tot_sold), y = tot_sold, label=tot_cust), nudge_y = 10)
```
```{r}
country_sales %>% mutate(Zone = case_when(country == "USA" ~ "USA", 
...
perc_sales %>% ggplot(aes(x=1, perc_sales, fill=Zone)) + geom_col(color="white", size=2) + coord_polar("y") + theme_void() + scale_fill_viridis_d(direction = -1) + geom_label(aes(label= sprintf("%1.1f%%", perc_sales)), position = position_stack(vjust=0.5), show.legend = FALSE) + ggtitle("Percentage of global sales")
```
```{r}
ggplot(filter(country_sales, c_group == "Normal"), aes(reorder(country, tot_sales_per_cust), tot_sales_per_cust, fill=tot_sales_per_cust)) + geom_col(show.legend = FALSE) + scale_fill_viridis_c() + theme(axis.text.x = element_text(angle = 90)) + scale_y_continuous(labels = label_dollar()) + labs(title="Average total sales per customer", subtitle="Countries with two or more clients", x="",y="")
```
